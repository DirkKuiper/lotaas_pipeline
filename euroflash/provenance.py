...
def reconcile(directory, ledger):
    ...
    for marker in directory.rglob('*.extracted.json'):
        ...
        if any(not Path(path).is_file() for path in value['fits']):
...
def coverage(ledger):
    with ledger.connect() as db:
        rows = db.execute('''WITH beam_history AS (
            ...
            LEFT JOIN runs r USING(fingerprint) GROUP BY b.uri,b.item
        ), archive_history AS (
        ...
        result['scope'] = 'Beam identity history across recorded production runs. Does not assert byte equivalence of archive replicas or scientific validation of every run.'
        return result
