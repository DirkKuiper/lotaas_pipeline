...
from src import filterbank
from src import plotting
from src import matched_filter
from src import matched_filter_gpu
from src import cluster
from src import classify
from src.inject import inject_synthetic_pulse

try:
    from src.cupy_utils import fourier_domain_dedispersion, compute_rfi_mask
    print("Using cupy")
except ModuleNotFoundError:
    print("Using numpy")
    from src.numpy_utils import fourier_domain_dedispersion, compute_rfi_mask

if __name__ == "__main__":
    # Input file
    fname = sys.argv[1]
    prefix = fname.replace(".fil", "")
    ...
    # This is where we want to save our outputs
    # Get the base filename without the path
    base_fname = os.path.basename(fname).replace(".fil", "")
    output_dir = f"output/{base_fname}"  # Main folder based on basename
   ...
   # Masking and replacing data with random noise
    ...
    # Assume `masked_data` is your input data (2D array: [freq, time])
    ...
    for entry in dedispersion_plan:
       ...
       # Folder to save all DM trials
        ...
        for i, dm in enumerate(dms):
            ...
            # print(f"Saved DM={dm:.1f} to {dm_filename}.dat and .inf")


    # Run matched filtering across all DM trials and generate an overview plot
    matched_filter_gpu.run_all_matched_filtering(dm_trials_dir, tsamp, output_dir, observation_info, dedispersion_plan)

    # Define the detected candidates file and the clustered output file
    ...
    classify.classify_candidates(fname, clustered_output_file, classified_output_dir)
