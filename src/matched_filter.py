...
def compute_filter_widths(tsamp, downsample, max_duration=600):
        ...
        """
        min_width = 2  # Smallest width to test in samples
...
def run_matched_filtering(data_file, tsamp, dm, downsample=1, detection_threshold=5):
    ...
    for i, width_samples in enumerate(filter_widths_samples):
        width_seconds = width_samples * tsamp  # Convert to seconds
...
def run_all_matched_filtering(dm_trials_dir, tsamp, output_dir, observation_info, downsampling_map, detection_threshold=5):
    all_candidates = []
    ...
    for dm_file in tqdm(dm_files, desc="Matched Filtering Progress", unit="file"):
        ...
        if match:
            ...
            downsample = downsampling_map.get(dm, 1)  # Default to 1 if not found
